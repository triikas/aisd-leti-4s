...
class MyShape : public rotatable, public reflectable {
  ...
  public:
    ...
    point east() const { return point((se.x + nwest().x) / 2, (nw.y + se.y) / 2); };
...
//        ne.y += b;
        try {
            sw.x += a;
            sw.y += b;
            if (!on_screen(sw.x, sw.y)) {throw CantBeMoved(sw.x, sw.y);}
            ne.x += a;
            ne.y += b;
            if (!on_screen(ne.x, ne.y)) {throw CantBeMoved(ne.x, ne.y);}
            se.x += a;
            se.y += b;
            if (!on_screen(se.x, se.y)) {throw CantBeMoved(se.x, se.y);}
            nw.x += a;
            nw.y += b;
            if (!on_screen(nw.x, nw.y)) {throw CantBeMoved(nw.x, nw.y);}
        } catch (CantBeMoved& ex) {
            ex.show();
        }
    }
